...
router.post("/addnote", fetchUser, async (req, res) => {
    try {
        ...
        const newNote = new Notes({ title, description, tag, user: req.userId });
...
router.put("/updatenote/:id", fetchUser, async (req, res) => {
    ...
    try {
        ...
        const updatedNote = await Notes.findByIdAndUpdate(id, { title, description, tag }, { new: true });

        res.json({ updatedNote, success: "Note updated successfully" });
